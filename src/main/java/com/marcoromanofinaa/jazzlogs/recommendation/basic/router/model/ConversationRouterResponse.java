...
public record ConversationRouterResponse(
        ...
        List<@Size(min = 1) String> excludedWinners
) {
    ...
    @AssertTrue(message = "must not include excludedWinners outside MUSIC_RECOMMENDATION")
    boolean hasExcludedWinnersConsistency() {
        return route == ConversationRoute.MUSIC_RECOMMENDATION || excludedWinners == null;
    }
}
