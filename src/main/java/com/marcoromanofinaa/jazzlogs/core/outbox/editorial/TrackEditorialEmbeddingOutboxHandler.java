...
public class TrackEditorialEmbeddingOutboxHandler implements OutboxEventHandler {
    ...
    public boolean supports(OutboxEventType type) {
        return type == OutboxEventType.TRACK_EDITORIAL_EMBEDDING_SYNC_REQUESTED;
    }

    @Override
    public void handle(OutboxEvent event) {
        try {
            TrackEditorialEmbeddingPayload payload = objectMapper.readValue(
                    event.getPayload(),
                    TrackEditorialEmbeddingPayload.class
            );

            editorialEmbeddingSyncService.syncTrackEmbedding(payload.trackNodeId());
        } catch (JsonProcessingException exception) {
            throw new OutboxEventPayloadDeserializationException(
                    "Failed to deserialize track editorial embedding payload",
                    exception
            );
        }
    }
