...
public class ArtistEditorialEmbeddingOutboxHandler implements OutboxEventHandler {
    ...
    public boolean supports(OutboxEventType type) {
        return type == OutboxEventType.ARTIST_EDITORIAL_EMBEDDING_SYNC_REQUESTED;
    }

    @Override
    public void handle(OutboxEvent event) {
        try {
            ArtistEditorialEmbeddingPayload payload = objectMapper.readValue(
                    event.getPayload(),
                    ArtistEditorialEmbeddingPayload.class
            );

            editorialEmbeddingSyncService.syncArtistEmbedding(payload.artistNodeId());
        } catch (JsonProcessingException exception) {
            throw new OutboxEventPayloadDeserializationException(
                    "Failed to deserialize artist editorial embedding payload",
                    exception
            );
        }
    }
