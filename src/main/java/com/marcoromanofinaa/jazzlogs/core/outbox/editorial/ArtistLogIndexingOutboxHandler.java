...
public class ArtistLogIndexingOutboxHandler implements OutboxEventHandler {
    ...
    public boolean supports(OutboxEventType type) {
        return type == OutboxEventType.ARTIST_LOG_INDEXING_REQUESTED;
    }

    @Override
    public void handle(OutboxEvent event) {
        try {
            ArtistLogPayload payload = objectMapper.readValue(
                    event.getPayload(),
                    ArtistLogPayload.class
            );

            editorialIndexingService.indexArtistLog(payload.artistLogId());
        } catch (JsonProcessingException exception) {
            throw new OutboxEventPayloadDeserializationException(
                    "Failed to deserialize ArtistLog outbox payload",
                    exception
            );
        }
    }
